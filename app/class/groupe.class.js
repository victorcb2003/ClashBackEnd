...
module.exports = class Groupe {
        ...
        pool.execute(sql, values, (err, results, fields) => {
            ...
            pool.execute(sql, values, (err, results) => {
                ...
                pool.execute(sql, values, (err) => {
                    ...
                    return res.status(201).send({ message: "Le groupe a bien été créé" })
                })
            })
        })
    }
    ...
    static add(req, res) {
        ...
        pool.query(sql, values, (err, results, fields) => {
            if (err) {
                return res.status(500).send({ error: "Une erreur s'est produite lors de l'ajout du membre au groupe." + err.message })
            }

            if (results.length == 0 && req.tokenData.type != "Admin") {
            ...
            pool.execute(sql, values, (err, results, fields) => {
                if (err) {
                    return res.status(500).send({ error: "Une erreur s'est produite lors de l'ajout du user au groupe. " + err.message })
                }
    ...
    static info(req, res) {
        ...
        pool.query(sql, values, (err, results, fields) => {
            if (err) {
                return res.status(500).send({ error: "Une erreur s'est produite lors de la récupération des info du groupe " + err.message })
            }
            if (results[0].length == 0) {
                return res.status(400).send({ error: "Il y a aucun groupe avec cette id" })
            }
    ...
    static findAll(req, res) {
        ...
        pool.execute(sql, (err, groupes, fields) => {
            ...
            if (groupes.length == 0) {
                return res.status(400).send({ error: "Il y a aucun groupe" })
            }
    ...
    static delete(req, res) {
        ...
        pool.execute(sql, values, (err, results, fields) => {
            if (err) {
                return res.status(500).send({ error: "Une erreur s'est produite lors de la suppression du groupe " + err.message })
            }
            if (results.length == 0) {
                return res.status(400).send({ error: "Il y a aucun groupe avec cette id" })
            }
            if (results[0].Owner_id != req.tokenData.id && req.tokenData.type != "Admin") {
                return res.status(403).send({ error: "Vous ne pouvez pas supprimer ce groupe" })
            }
    ...
    static remove(req, res) {
        ...
        pool.query(sql, values, (err, results, fields) => {
            if (err) {
                return res.status(500).send({ error: "Une erreur s'est produite lors de la récupération de la suppression du user du groupe " + err.message })
            }
            if (results.length == 0) {
                return res.status(400).send({ error: "Il y a aucun groupe avec cette id" })
            }
            if (results[0][0].Owner_id != req.tokenData.id && req.tokenData.type != "Admin") {
                return res.status(403).send({ error: "Vous n'etes pas le propriétaire de ce groupe." })
            }
    ...
    static rename(req, res) {
        ...
        pool.execute(sql, values, (err, results, fields) => {
            if (err) {
                return res.status(500).send({ error: "Une erreur s'est produite lors du changement de nom du groupe " + err.message })
            }
            if (results.length == 0) {
                return res.status(400).send({ error: "Il y a aucun groupe avec cette id" })
            }
            if (results[0].Owner_id != req.tokenData.id && req.tokenData != "Admin") {
                return res.status(403).send({ error: "Vous ne pouvez pas changer le nom de ce groupe" })
            }
    ...
    static async messageCreate(req, res) {
        ...
        let sql = "Select id from Groupes_Membres where User_id = ?"
        let values = [req.tokenData.id]
        ...
        pool.execute(sql, values, (err, result) => {
            ...
            if (result.length == 0 && req.tokenData.type != "Admin") {
                return res.status(403).send({ error: "Vous ne faites pas partie de ce groupe" })
            }

            sql = "Insert into Groupes_Messages (expediteur_id,Groupe_id,message) values (?,?,?)"
            values = [req.tokenData.id, req.body.groupe_id, req.body.message]
            pool.execute(sql, values, (err, results, fields) => {
                if (err) {
                    return res.status(500).send({ error: "Une erreur s'est produite lors de la création du message. " + err.message })
                }
    ...
    static messageDelete(req, res) {
        ...
        pool.execute(sql, values, (err, results, fields) => {
            ...
            if (results.length == 0) {
                return res.status(400).send({ error: "aucun message avec cette id" })
            }
            if (results[0].expediteur_id != req.tokenData.id && req.tokenData != "Admin") {
    ...
    static messageFindAll(req, res) {
        ...
        pool.execute(sql, values, (err, result) => {
            ...
            sql = "SELECT Groupes_Messages.* FROM Groupes_Messages JOIN Groupes_Membres ON Groupes_Messages.Groupe_id = Groupes_Membres.Groupe_id WHERE Groupes_Membres.User_id = ?"
