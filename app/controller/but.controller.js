...
exports.create = (req, res) => {
    ...
    if (!req.body.Type_But){
        return res.status(400).send({error : "req.body.Type_But est requis."})
    }
    const regex = /^\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01]) ([01]\d|2[0-3]):[0-5]\d:[0-5]\d$/;

    if (!regex.test(req.body.date_heure)) {
        return res.status(400).send({ error: "La date doit être au format 'YYYY-MM-DD HH:mm:SS'."  })
    }
    ...
    if (parseInt(req.body.User_id) != req.body.User_id){
        return res.status(400).send({error : "req.body.User_id est pas un entier"})
    }
...
exports.info = (req,res)=>{
    ...
    if (typeof(req.params.id) != "number" || parseInt(req.params.id) != req.params.id){
        return res.status(400).send({error : "req.params.id est pas un entier"})
    }
...
exports.delete = (req,res) =>{
    ...
    if (!req.params.id){
        return res.status(400).send({error : "req.body.But_id est requis."})
    }
    if (parseInt(req.params.id) != req.params.id){
        return res.status(400).send({error : "req.body.But_id est pas un entier"})
    }
...
exports.update = (req,res) =>{
    ...
    if (!req.body.date_heure && !req.body.User_id && !req.body.Match_id && !req.body.Type_But){
        return res.status(400).send({ error: "il faut au moins remplir req.body.date_heure ou req.body.User_id ou req.body.Match_id ou req.body.Type_But"  })
    }
    const regex = /^\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01]) ([01]\d|2[0-3]):[0-5]\d:[0-5]\d$/;
